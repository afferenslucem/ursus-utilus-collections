...
import { MappingManyCollection, Sequence } from '../../src/collection';

describe('MappingManyCollection', function () {  
    it('should create', () => {
        const result = new MappingManyCollection(new Sequence([1, 2, 3]), item => [item]);
    });

    it('should return mapped from toArray()', () => {
        const result = new MappingManyCollection(new Sequence([8, 5, 4, 2, 9, 1, 4]), item => [item + 1]).toArray();
    ...
    it('should return double mapped from toArray()', () => {
        const result = new MappingManyCollection(new Sequence([[8], [5], [4], [2], [9], [1], [4]]), item => [item]).selectMany(item => item.map(a => (Number(a) + 1) / 2)).toArray();
    ...
    it('should return empty for empty collection', () => {
        const result = new MappingManyCollection(new Sequence([]), item => [item + 1]).select(item => item).toArray();
