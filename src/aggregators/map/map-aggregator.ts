...
export class MapAggregator<T, TKey, TValue = T> extends Aggregator<Map<TKey, TValue>> {
    ...
    public aggregate(): Map<TKey, TValue> {
        const grouped = this.valueSelector != null ? 
            // @ts-ignore
            this.collection.groupBy(this.keySelector, group => group.select(this.valueSelector).first()) :
            this.collection.groupBy(this.keySelector, group => group.first());
