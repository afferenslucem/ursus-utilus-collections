...
export class ReduceAggregator<T, V = T> extends Aggregator<V> {
    ...
    protected chooseAlgorithm<V>(array: T[]): IAlgorithm<V> {
        if(!this.accumulator) {
            return AlgorithmSolver.solve(new ReduceCustomAlgorithm<T, V>(), new ReduceNativeAlgorithm<T, V>(), array);
        } else {
            // @ts-ignore
            return AlgorithmSolver.solve(new ReduceWithAccumulatorCustomAlgorithm<T, V>(), new ReduceWithAccumulatorNativeAlgorithm<T, V>(), array);
        }
    }
}